...
let maxParsedSuggestionId = 0;
    ...
    const sections = {
        ...
        REVERSE_EDIT_SUGGESTIONS: "--- REVERSE EDIT SUGGESTIONS ---" 
    };
...
if (condIdsMatch && condIdsMatch[1].trim()) suggestionData.conditionCharIds = new Set(condIdsMatch[1].trim().split(',').filter(id => id));
    ...
    const reverseEditSuggestionsContent = extractSectionContent(sections.REVERSE_EDIT_SUGGESTIONS);
    if (reverseEditSuggestionsContent) {
        const suggestionBlocks = reverseEditSuggestionsContent.split("--- SUGGESTION ");
        suggestionBlocks.forEach(block => {
            if (!block.trim() || !/^(-?\d+)\s*---/.test(block.trimStart())) return; 
            const suggestionData = { removedCharIds: new Set(), newCharIds: new Set(), conditionCharIds: new Set(), score: 0, selectedDraftAtTimeOfEdit: [], resultingDraft: [] }; 
            const idMatch = block.match(/^(-?\d+)\s*---/); 
            ...
            parsedOutput.reverseEditSuggestions.push(suggestionData);
        });
    }
...
const allDraftsListRef = useRef(null); 
  ...
  const [reverseEditSuggestions, setReverseEditSuggestions] = useState([]); 
...
null,
  ...
  const suggestionsForSelectedDraft = useMemo(() => {
    ...
    const currentDraftKey = getDraftKey(selectedDraft);
    if (!currentDraftKey) {
      return [];
    }
    const forward = editSuggestions.filter(
      sugg => getDraftKey(sugg.selectedDraftAtTimeOfEdit) === currentDraftKey
    );
    const reverse = reverseEditSuggestions.filter(
      sugg => getDraftKey(sugg.selectedDraftAtTimeOfEdit) === currentDraftKey
    );
...
useEffect(() => {
  ...
  }, [history, redoStack, drafts, editSuggestions, draftVectorsMap, reverseEditSuggestions]);
function saveHistory(newDraftsData, newEdgesData) {
    setHistory(h => [...h, { drafts: drafts, suggestions: editSuggestions, draftVectors: draftVectorsMap, reverseSuggestions: reverseEditSuggestions }]); 
    setRedoStack([]); 
...
setGraphEdges(e => [...e, ...newEdgesData]);
  ...
  function undo() {
    ...
    setRedoStack(r => [{ drafts: drafts, suggestions: editSuggestions, draftVectors: draftVectorsMap, reverseSuggestions: reverseEditSuggestions }, ...r]); 
const prevState = history[history.length - 1];
    ...
    setReverseEditSuggestions(prevState.reverseSuggestions || []); 
const newSelectedDraft = prevState.drafts[0] || [];
    ...
    if (showSuggestionsDialog) { 
        ...
        const newTotalSuggestions = prevForward.length + prevReverse.length;

        if (newTotalSuggestions === 0) {
            setShowSuggestionsDialog(false);
  setCurrentSuggestionViewIndex(0);
        } else {
            setCurrentSuggestionViewIndex(prev => Math.min(prev, newTotalSuggestions - 1));
        }
    }
  }
...
setRedoStack(r => r.slice(1));
    ...
    setReverseEditSuggestions(nextState.reverseSuggestions || []); 
    ...
    if (showSuggestionsDialog) { 
        ...
        const newTotalSuggestions = nextForward.length + nextReverse.length;
...
}} let anchorSegmentText = null; let anchorSegmentEndIndex = -1; const sentenceBoundaryRegex = /[^.?!;:\n]+(?:[.?!;:\n]|$)|[.?!;:\n]/g; let matchBoundary; sentenceBoundaryRegex.lastIndex = 0;
...
}} if (anchorSegmentText !== null) { const trimmedSegment = anchorSegmentText.trim().replace(/\n$/, ''); const isTrueSentence = /[.?!;:]$/.test(trimmedSegment);
if (isTrueSentence) { insertionPointInDArr = anchorSegmentEndIndex + 1; } else { insertionPointInDArr = anchorIdIndexInDArr + 1;
...
if (insertionPointInDArr > targetDraftText.length) insertionPointInDArr = targetDraftText.length; } while (insertionPointInDArr < targetDraftText.length && targetDraftText.charAt(insertionPointInDArr) === '\n') insertionPointInDArr++;} 
...
setEditSuggestions(prevSuggestions => [...prevSuggestions, newSuggestionEntry]); 
    ...
    const newReverseSuggestionEntry = {
        id: -newSuggestionEntry.id, 
        selectedDraftAtTimeOfEdit: newSuggestionEntry.resultingDraft, 
        resultingDraft: newSuggestionEntry.selectedDraftAtTimeOfEdit, 
        removedCharIds: newSuggestionEntry.newCharIds, 
        newCharIds: newSuggestionEntry.removedCharIds, 
        conditionCharIds: new Set(), 
        score: -newSuggestionEntry.score 
    };
    setReverseEditSuggestions(prevReverseSuggestions => [...prevReverseSuggestions, newReverseSuggestionEntry]);
...
editSuggestions.forEach(suggestion => {
    ...
    fileContent += "--- REVERSE EDIT SUGGESTIONS ---\n\n";
    reverseEditSuggestions.forEach(suggestion => {
        fileContent += `--- SUGGESTION ${suggestion.id} ---\n`;
        fileContent += `Score: ${suggestion.score}\n`;
        fileContent += `SelectedDraftAtTimeOfEdit:\n  ${serializeCharObjArray(suggestion.selectedDraftAtTimeOfEdit)}\n`;
        fileContent += `ResultingDraft:\n  ${serializeCharObjArray(suggestion.resultingDraft)}\n`;
...
if (parsed.maxSeenId >= 0) globalCharCounter = parsed.maxSeenId + 1; else globalCharCounter = 0; 
            ...
            setReverseEditSuggestions(parsed.reverseEditSuggestions || []); 
 ...
 >
      ...
      {suggestionsForSelectedDraft.length > 0 && (
          ...
          onClick={() => { setCurrentSuggestionViewIndex(0); setShowSuggestionsDialog(true);
              ...
              >
                View Edits for This Draft ({suggestionsForSelectedDraft.length})
              </button>
          </div>
      )}
      
      {showSuggestionsDialog && suggestionsForSelectedDraft.length 
> 0 && (
        <SuggestionsDialog 
          suggestions={suggestionsForSelectedDraft} 
          currentIndex={currentSuggestionViewIndex} 
          onClose={() => setShowSuggestionsDialog(false)} 
          onNext={() => setCurrentSuggestionViewIndex(prev => Math.min(prev + 1, suggestionsForSelectedDraft.length - 1))} 
...
<div className="lg:flex-1 w-full mb-6 lg:mb-0">
                ...
                <ul ref={allDraftsListRef} className="flex flex-wrap gap-2 justify-center bg-gray-50 p-3 rounded-md shadow max-h-[400px] overflow-y-auto">
                   {displayStringDrafts.map((text, i) => {
                       const actualDraftCharArr 
= sortedAndVisibleDrafts[i];
...
"Undo Draft Score Sort" : "Sort by Draft Score"}
    ...
    (() => {
                            ...
                            const vector = draftVectorsMap.get(draftKey);
                            const score = calculateDraftScore(selectedDraft, 
draftVectorsMap, editSuggestions);
                            let displayString = "";
