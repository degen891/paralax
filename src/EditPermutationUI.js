...
let maxParsedSuggestionId = 0;
    ...
    const sections = {
        ...
        REVERSE_EDIT_SUGGESTIONS: "--- REVERSE EDIT SUGGESTIONS ---" // <-- ADDED
    };
...
if (condIdsMatch && condIdsMatch[1].trim()) suggestionData.conditionCharIds = new Set(condIdsMatch[1].trim().split(',').filter(id => id));
    ...
    // --- ADDED BLOCK for REVERSE EDIT SUGGESTIONS ---
    ...
    if (reverseEditSuggestionsContent) {
        ...
        suggestionBlocks.forEach(block => {
            if (!block.trim() || !/^(-?\d+)\s*---/.test(block.trimStart())) return; // Allow negative IDs
            const suggestionData = { removedCharIds: new Set(), newCharIds: new Set(), conditionCharIds: new Set(), score: 0, selectedDraftAtTimeOfEdit: [], resultingDraft: [] }; 
            const idMatch = block.match(/^(-?\d+)\s*---/); // Allow negative IDs
            if (idMatch) suggestionData.id = parseInt(idMatch[1], 10); else return;
            
            // Note: We do not update maxParsedSuggestionId for reverse suggestions
...
const [hideBadEditsActive, setHideBadEditsActive] = useState(false);
  const [reverseEditSuggestions, setReverseEditSuggestions] = useState([]); // <-- ADDED
...
useEffect(() => {
  ...
  }, [history, redoStack, drafts, editSuggestions, draftVectorsMap, reverseEditSuggestions]); // <-- MODIFIED
function saveHistory(newDraftsData, newEdgesData) {
    setHistory(h => [...h, { drafts: drafts, suggestions: editSuggestions, draftVectors: draftVectorsMap, reverseSuggestions: reverseEditSuggestions }]); // <-- MODIFIED
    setRedoStack([]); 
...
setGraphEdges(e => [...e, ...newEdgesData]);
  ...
  function undo() {
    ...
    setRedoStack(r => [{ drafts: drafts, suggestions: editSuggestions, draftVectors: draftVectorsMap, reverseSuggestions: reverseEditSuggestions }, ...r]); // <-- MODIFIED
const prevState = history[history.length - 1];
    ...
    setReverseEditSuggestions(prevState.reverseSuggestions || []); // <-- ADDED
...
setRedoStack(r => r.slice(1));
    ...
    setReverseEditSuggestions(nextState.reverseSuggestions || []); // <-- ADDED
...
setDraftVectorsMap(new Map([[initialDraftKey, [1]]])); 
    ...
    setConditionParts([]);
    setEditSuggestions([]);
    setReverseEditSuggestions([]); // <-- ADDED
...
setEditSuggestions(prevSuggestions => [...prevSuggestions, newSuggestionEntry]); 
    ...
    // --- ADDED BLOCK for REVERSE SUGGESTION ---
    const newReverseSuggestionEntry = {
        id: -newSuggestionEntry.id, // Use negative ID for the reverse
        selectedDraftAtTimeOfEdit: newSuggestionEntry.resultingDraft, // Swapped
        resultingDraft: newSuggestionEntry.selectedDraftAtTimeOfEdit, // Swapped
        removedCharIds: newSuggestionEntry.newCharIds, // Swapped
        newCharIds: newSuggestionEntry.removedCharIds, // Swapped
        conditionCharIds: new Set(), // Empty as requested
        score: -newSuggestionEntry.score // Negative score as requested
    };
...
if (parsed.maxSeenId >= 0) globalCharCounter = parsed.maxSeenId + 1; else globalCharCounter = 0; 
            ...
            setReverseEditSuggestions(parsed.reverseEditSuggestions || []); // <-- ADDED
...
return conditionParts.map(part => `'${part.text}'`).join(' + '); 
 ...
 >
      ...
      {showSuggestionsDialog && editSuggestions.length 
> 0 && (
        <SuggestionsDialog 
