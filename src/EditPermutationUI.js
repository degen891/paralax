...
// Find all starting indices where a sequence of IDs occurs in a CharObj[]
...
// Auto-conditions based on sentence context, returning arrays of ID sequences
function getAutoConditionsIds(arr, offset, removedLen) {
  ...
  if (!sequences.length) {
    sequences.push(arr.slice(paraStart, paraEnd).map(c => c.id));
  }
  return sequences;
}
...
export default function EditPermutationUI() {
  ...
  useEffect(() => {
    setCurrentEditText(
      selectedDraft.length ? charArrayToString(selectedDraft) : ""
    );
  }, [selectedDraft]);
  ...
  function applyEdit() {
    ...
    const removedIds =
      removedLen > 0
        ? oldArr
            .slice(prefixLen, prefixLen + removedLen)
            .map(c => c.id)
        : [];
    ...
    const newEdges = [];
    const seen = new Set(
      newDraftsArr.map(d => d.map(c => c.id).join(","))
    );

    drafts.forEach(dArr => {
      if (
        !condSeqs.every(seq =>
          findIdSeqPositions(dArr, seq).length > 0
        )
      )
        return;
      const variants = [];
      ...
      // INSERTIONS
      if (insertedText) {
        const insArr = Array.from(insertedText).map(ch => ({
          id: generateCharId(),
          char: ch,
        }));
        if (!removedIds.length) {
  ...
  function handleSelect() {
    ...
    const spans = Array.from(
      draftBoxRef.current.querySelectorAll("span[data-id]")
    );
    // helper to find enclosing span
    ...
    const multi =
      sel.getModifierState("Control") || sel.getModifierState("Meta");
  ...
  function renderEditableDraft(arr) {
    return (
      <div
        ...
        onInput={e =>
          setCurrentEditText(e.currentTarget.textContent)
        }
      ...
      >
        {arr.map(c => (
          <span
            ...
            data-id={c.id}
            className={
              highlightedIds.includes(c.id) ? "bg-yellow-200" : ""
            }
          >
            {c.char}
          </span>
